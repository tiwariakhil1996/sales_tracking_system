...
namespace STS.Controllers
{
    ...
    public class ClientController : Controller
    {
        ...
        public async Task<IActionResult> ChangeStatusClient(int id)
        {
            Dictionary<String, Object> dctData = new Dictionary<string, object>();
            HttpStatusCode statusCode = HttpStatusCode.OK;
            TranStatus transaction = new TranStatus();
            try
            {
                transaction = await iclient.ChangeStatusClient(id);
        ...
        [HttpGet]
        [Route("ClientList_ActiveDeactive")]
        public async Task<IActionResult> ClientList_ActiveDeactive()
        {
            TranStatus transaction = new TranStatus();
            Dictionary<String, Object> dctData = new Dictionary<string, object>();
            HttpStatusCode statusCode = HttpStatusCode.OK;
            try
            {
                var clientList = await iclient.ClientList_ActiveDeactive();
                dctData.Add("ClientList_ActiveDeactive", clientList);
            }
            catch (Exception ex)
            {
                transaction = CommonHelper.TransactionErrorHandler(ex);
